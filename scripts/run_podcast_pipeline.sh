...
# 按“下载全部音频 -> 转写全部音频 -> 提取公司”的顺序逐档处理播客。
# 用法：./scripts/run_podcast_pipeline.sh <播客名或RSS地址> [<播客名或RSS地址> ...]
...
if [[ $# -eq 0 ]]; then
  echo "用法：$0 <播客名或RSS地址> [<播客名或RSS地址> ...]" >&2
  exit 2
fi
...
for podcast_reference in "${PODCAST_REFERENCES[@]}"; do
  shopt -s nocasematch
  ...
  echo "===== 开始处理播客：${podcast_reference} =====" | tee -a "$LOG_FILE"

  # 下载器会跳过已经存在的非空音频。如果上一次运行已经删除了已完成节目的音频，
  # 这些节目会重新下载；后面的转写阶段遇到完整转写结果时仍会跳过。
  # 每个阶段末尾的“|| exit $?”表示：run_stage 成功返回 0 时继续下一阶段；返回
  # 非零退出码时，exit 使用这个退出码结束整个脚本，因此当前播客的后续阶段和
  # 尚未开始的播客都不会执行。
  run_stage "下载全部音频" \
